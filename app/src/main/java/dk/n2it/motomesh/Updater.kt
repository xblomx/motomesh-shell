...
object Updater {
    ...
    fun force(act: Activity) {
        if (!BuildConfig.SELF_UPDATE) {
            // Play build: updates come from Google Play · the update chip opens the store listing.
            try { act.startActivity(Intent(Intent.ACTION_VIEW, Uri.parse("market://details?id=" + act.packageName))) }
            catch (_: Exception) { try { act.startActivity(Intent(Intent.ACTION_VIEW, Uri.parse("https://play.google.com/store/apps/details?id=" + act.packageName))) } catch (_: Exception) {} }
            return
        }
        lastCheck = 0L; check(act, true)
    }
